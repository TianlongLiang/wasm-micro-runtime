...
ZTEST(platform_thread, test_create_and_join)
{
    ...
    zassert_equal(os_thread_create(&thread, write_result, &result,
...
{
    zassert_equal(os_thread_create(NULL, write_result, NULL,
                                   WAMR_TEST_STACK_SIZE),
                  BHT_ERROR, NULL);
}
...
ZTEST(platform_thread, test_stack_pool_recovers_after_exhaustion)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(threads); ++i) {
        create_result = os_thread_create(&threads[i],
                                         block_for_stack_recovery, &state,
                                         WAMR_TEST_STACK_SIZE);
    ...
    zassert_true(exhausted, "stack allocation did not fail before attempt nine");
