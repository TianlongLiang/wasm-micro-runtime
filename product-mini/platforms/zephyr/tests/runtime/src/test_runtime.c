...
run_add_lifecycle(uint32_t *result)
{
    ...
    return wamr_test_run_add_copy(
        ...
        sizeof(runtime_fixture.pool), wasm_add, sizeof(wasm_add), result,
        error, sizeof(error));
}
...
ZTEST(runtime_interpreter_pool, test_initialize_and_destroy)
{
    ...
    failed_initialized = wamr_test_runtime_init(
        &failed_runtime, failed_pool, sizeof(failed_pool));
    ...
    initialized = wamr_test_runtime_init(&runtime_fixture.runtime,
                                         runtime_fixture.pool,
    ...
    if (started) {
        ...
        if (trap != NULL) {
            ...
            exception_set = wasm_runtime_get_exception(
                                runtime_fixture.runtime.instance)
                            != NULL;
        }
        wamr_test_runtime_stop(&runtime_fixture.runtime);
    }
...
ZTEST(runtime_interpreter_pool, test_missing_export_is_clean_failure)
{
    ...
    if (started) {
        missing =
            wasm_runtime_lookup_function(runtime_fixture.runtime.instance,
                                         "missing");
        exception_set = wasm_runtime_get_exception(
                            runtime_fixture.runtime.instance)
                        != NULL;
