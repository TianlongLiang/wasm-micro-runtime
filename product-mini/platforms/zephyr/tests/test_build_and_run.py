...
class CoverageWorkflowTest(unittest.TestCase):
    def test_coverage_job_uses_available_manifest_and_local_checkout(self):
        workflow = WORKFLOW.read_text(encoding="utf-8")
        coverage_job = workflow.split("  coverage_measurement:", 1)[1]
        manifest_line = next(
            line
            for line in coverage_job.splitlines()
            if "manifest-file-name:" in line
        )
        manifest_name = manifest_line.split(":", 1)[1].strip()

        self.assertTrue((SCRIPT.parent / manifest_name).is_file())
        self.assertIn(
            "cp -r modules/wasm-micro-runtime/product-mini/platforms/zephyr "
            "application",
            coverage_job,
        )
        self.assertIn(
            "project-filter = -wasm-micro-runtime", coverage_job
        )
